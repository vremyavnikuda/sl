...
int main() {
    try {
        ...
        const std::string config_file = "sl.config";

        // Чтение директории с музыкой из конфигурационного файла
        const std::string music_directory = Music::get_music_directory_from_config(config_file);
        ...
        // Получение плейлиста
        std::vector<std::string> playlist = Music::get_playlist(music_directory);
        ...
        std::thread player_thread(Music::play_playlist, std::ref(playlist), std::ref(track_index));
        ...
        while (running) {
            ...
            Ui::display_interface(playlist, track_index);
        }
